...
void MCP23017::writeRegister(uint8_t regAddr, uint8_t regValue) {
	// Check for modifications and return if no modifications have to be done:
	if(regAddr == MCP23017_GPIOA){
		if(regValue == GPOA_state){
			return;
		}
		GPOA_state = regValue;
	}
	if(regAddr == MCP23017_GPIOB){
		if(regValue == GPOB_state){
			return;
		}
		GPOB_state = regValue;
	}
 ...
 */
int8_t MCP23017::begin(uint8_t addr) {
  if (addr > 7) {
    addr = 7;
  }
  i2caddr = addr;
  ...
  if(Wire.endTransmission()){
	  Serial.print("ERROR @ MCP23017, addr="); Serial.println(MCP23017_ADDRESS | i2caddr);
	  return 1;
  }
 ...
 * address
 */
int8_t MCP23017::begin(void) { return begin(0); }
...
void MCP23017::digitalWrite(uint8_t pin, uint8_t d) {
  // Bit in GPOX for pin-number:
  uint8_t bit = bitForPin(pin);
  // read the current GPIO output latches out of storage, not out of chip:
  uint8_t gpio = (pin < 8) ? GPOA_state : GPOB_state;
  uint8_t gpio_ = gpio;
  // set the pin:
  bitWrite(gpio, bit, d);
  // Check for no changes:
  if(gpio_ == gpio){return;}
  // write the new GPIO
  uint8_t regAddr = regForPin(pin, MCP23017_GPIOA, MCP23017_GPIOB);
  writeRegister(regAddr, gpio);
  // Refresh stored gpio-state:
  if(regAddr == MCP23017_GPIOA){
	  GPOA_state = gpio;
  } else{
	  GPOB_state = gpio;
