 ...
 *
 * @author Michael Meli
 * @author edited by Thomas Thurner
 */
 ...
 */
int AD5933s::getByte(byte address, byte *value) {
 ...
 */
bool AD5933s::sendByte(byte address, byte value) {
 ...
 * @return Success or failure
 */
bool AD5933s::setControlMode(byte mode) {
...
/**
 * Reset the AD5933s. This interrupts a sweep if one is running, but the start
 ...
 * @return Success or failure
 */
bool AD5933s::enableTemperature(byte enable) {
 ...
 * @return Success or failure
 */
bool AD5933s::setClockSource(byte source) {
 ...
 * @return Success or failure
 */
bool AD5933s::setInternalClock(bool internal) {
 ...
 * @return Success or failure
 */
bool AD5933s::setStartFrequency(unsigned long start) {
 ...
 * @return Success or failure
 */
bool AD5933s::setIncrementFrequency(unsigned long increment) {
 ...
 * @return Success or failure
 */
bool AD5933s::setNumberIncrements(unsigned int num) {
 ...
 * @return Success or failure
 */
bool AD5933s::setPGAGain(byte gain) {
    // Get the current value of the control register
    byte val;
    if (!getByte(CTRL_REG1, &val))
        return false;
...
bool AD5933s::setSettlingTimeCycles(unsigned int setlingCycles){
	
	byte val1 = 0;
	byte val2 = 0;
	if(setlingCycles > 2047)		{ val1 = 0b111; val2 = 0xFF; }
	else if(setlingCycles > 1023)	{ val1 = 0b111; val2 = (uint8_t)((setlingCycles/4) & 0xFF); }
	else if(setlingCycles > 511)	{ val1 = 0b011; val2 = (uint8_t)((setlingCycles/2) & 0xFF); }
	else if(setlingCycles > 255)	{ val1 = 0b001; val2 = (uint8_t)((setlingCycles  ) & 0xFF); }
	else							{ val1 = 0b000; val2 = (uint8_t)((setlingCycles  ) & 0xFF); }
 ...
 */
byte AD5933s::readRegister(byte reg) {
 ...
 */
byte AD5933s::readStatusRegister() {
 ...
 */
int AD5933s::readControlRegister() {
 ...
 * @return Success or failure
 */
bool AD5933s::getComplexData(int16_t *real, int16_t *imag) {
    // Wait for a measurement to be available
	
	//unsigned long a = micros();
    while ((readStatusRegister() & STATUS_DATA_VALID) != STATUS_DATA_VALID);
	//Serial.print(micros()-a); Serial.println(" us of waiting-time...");
...
/**
 * Set the power level of the AD5933s.
 *
 * @param level The power level to choose. Can be on, standby, or down.
 * @return Success or failure
 */
bool AD5933s::setPowerMode(byte level) {
...
// Begin performing endless single frequency measures:
bool AD5933s::beginOneFrequency(){
// Begin by issuing a sequence of commands if not yet begun:
...
/**
 * Perform a single Frequency Measurement again and again...
 *
 * @param real An integer to hold the real data.
 * @param imag An integer to hold the real data.
 * @param state Pointer to state of the procedure ['-1'--> OFF, '0'--> Running, '1'--> Turn OFF]
 * @return Success or failure
 */

bool AD5933s::getOneFrequency(int16_t* real, int16_t* imag){
	// Repeat the frequency and our index.
	setControlMode(CTRL_REPEAT_FREQ);
	// Get the data for this frequency point and store it in the array
	if (!getComplexData(&tempRealData, &tempImagData)) {
		// In case of Error:
		setPowerMode(POWER_STANDBY);
		return false;
	}
	if (real != nullptr){
		*real = tempRealData;
...
/*bool AD5933s::getOneFrequency(float* magnitude){
	// Repeat the frequency and our index.
	setControlMode(CTRL_REPEAT_FREQ);
	// Get the data for this frequency point and store it in the array
	if (!getComplexData(&tempRealData, &tempImagData)) {
	...
	if(magnitude != nullptr){
		*magnitude = (float) sqrt(pow(tempRealData, 2) + pow(tempImagData, 2));
	}
	return true;
}*/

// Begin performing endless single frequency measures:
bool AD5933s::endOneFrequency(){
// End the procedure
 ...
 * @return Success or failure
 */
bool AD5933s::frequencySweep(int real[], int imag[], int n) {
 ...
 * @return Success or failure
 */
bool AD5933s::calibrate(double gain[], int phase[], int ref, int n) {
 ...
 * @return Success or failure
 */
bool AD5933s::calibrate(double gain[], int phase[], int real[], int imag[],
                       int ref, int n) {
    // Perform the frequency sweep
    if (!frequencySweep(real, imag, n)) {
        return false;
